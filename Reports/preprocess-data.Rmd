...
```{r loadSdrf}
cell.meta <- read_tsv('../extdata/E-MTAB-3929.sdrf.txt', progress = FALSE)
...
Load counts.
```{r loadCounts}
system.time(counts <- read_tsv('../extdata/counts.txt', progress = FALSE))
colnames(counts)[1] <- 'gene'
log.expr <- log10(as.matrix(counts[,2:ncol(counts)]) + 1)
dimnames(log.expr) <- list(gene = counts$gene, cell = colnames(counts)[2:ncol(counts)])
```

Melt counts to long format.
```{r meltExpr}
expr.m <-
    melt(log.expr, value.name = 'log.x') %>%
...
gene.meta <-
    expr.m %>%
    group_by(gene) %>%
    summarise(mean = mean(log.x), var = var(log.x)) %>%
    arrange(-var / mean)
head(gene.meta)
...
calc.cell.sizes <- function(.data) {
    gene.means <-
        .data %>%
        group_by(gene) %>%
        summarise(gene.mean = mean(log.x))
    .data %>%
        left_join(gene.means) %>%
        group_by(cell) %>%
        summarise(size = median(log.x - gene.mean))
}
...
embryonic.day.aov <-
    .data %>%
    group_by(gene) %>%
    do(tidy(aov(log.x.adj ~ lineage, .))) %>%
    ungroup() %>%
    arrange(p.value)
embryonic.day.aov %>% filter(p.value <= .05)
```
